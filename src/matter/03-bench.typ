...
The cavity is excited by electric dipole sources (transmitters) located at points $zv in Lambda$
with polarization vectors $p in RR^3$. The incident electric field generated by
such a transmitter dipole in free space is given by
$
...
The boundary forcing term is then given by
$
  avec(h) = -nv times Ev^i quad "on" partial D
$


Scattered electric field $Ev^s: D -> CC^3$ satisfies
$
  curl curl Ev^s - k^2 Ev^s &= 0 quad "in" D, \
  nv times Ev^s &= avec(h) quad "on" partial D.
$
Where $nv$ denotes the outward unit normal on $partial D$, and $avec(h)$
the boundary forcing term.


Measurements are modeled as tangential trace data on the interior surface $Lambda$.
$
  yv(xv) = nv_Lambda (xv) times Ev^s (xv), quad xv in Lambda
$
where $nv_Lambda$ denotes the outward unit normal on $Lambda$.
